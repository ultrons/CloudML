...
Orignal Source:
    https://raw.githubusercontent.com/tensorflow/tpu/master/tools/datasets/jpeg_to_tf_record.py

Change Summary:
    - Mapping to max dimensions and resizing to target size added

Note:
    - Dataflow quits silently if the input images are varying dimensions.
    (To be investigated)
"""
...
class ImageCoder(object):
  ...
  def __init__(self):
    ...
    # VS: Scale to the Max size
    # This can introduce black padding lines in smaller images
    # However this ensure that the aspect ratio of objects inside the images
    # changes similarly
    #self._decode_jpeg =
    #tf.image.resize_image_with_crop_or_pad(self._decode_jpeg, max_height,
    #        max_width)
    self._decode_jpeg = tf.image.resize_images(self._decode_jpeg,
            size=[299,299], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
  ...
  def decode_jpeg(self, image_data):
    ...
    assert image.shape[1] == 299, "Actual shape width=%d" %(image.shape[1])
    assert image.shape[0] == 299, "Actual shape width=%d" %(image.shape[0])
    return image
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--target_width', help='width of scaled output images', default = 299, type=int)
  parser.add_argument(
      '--target_height', help='height of scaled output images', default = 299, type=int)
  parser.add_argument(
      '--max_width', help='Max-width in the dataset', default = 1732, type=int)
  parser.add_argument(
      '--max_height', help='Max-height in the dataset', default = 974, type=int)
